...
int main()
{
	...
	int algo;
	char msg[4][15] = {"Merge Sort", "Quick Sort", "Selection Sort", "Two for Loops"};
	
	// Choose sort algorithm
	//=========================================================================================
	printf("Please choose a sorting algorithm: \n");
	printf("[1] Merge Sort (Best) [default]\n[2] Quick Sort (Fast)\n[3] Selection Sort (Slow)\n[4] Two for Loops (slower)\n");
	scanf("%d", &algo);
	...
	//=========================================================================================
	printf("\nFinding the 10 closest vehicles (%s algo)\n", msg[algo-1]);
	start_t = clock();
	for(int i = 0; i < 10; i++){
		if(algo == 2){
			quickSort(vehicles, 0, 2000000 - 1, i);				// Sort
		}else
		if(algo == 3){
			selectionSort(vehicles, i);							// Sort
		}else
		if(algo == 2){
			sort2loops(vehicles, i);							// Sort
		}
		else{
